...
**Mise à jour (2026-08-07) — ce correctif est incomplet, pas faux mais partiel** : `--import-mode=importlib` résout la collision de **collecte** des fichiers de test (deux `conftest.py`/`test_x.py` de même nom), mais **pas** la collision d'**import du code testé lui-même**. Si plusieurs Lambdas ont chacune un `auth.py`/`db.py`/`errors.py` (imports plats, pas de package — imposé par le runtime Lambda), un `import auth` dans un fichier de test reste résolu via le `sys.modules` global du process Python, indexé par le nom nu `"auth"` — peu importe le mode de collecte pytest. Le premier `import auth` exécuté pendant la session (déterminé par l'ordre de collecte, pas par le dossier du test qui l'exécute) fige quel `auth.py` sera utilisé par **tous** les `import auth` suivants, dans **tous** les dossiers, pour le reste du run. Ce bug reste **invisible** tant que les fichiers de même nom entre Lambdas sont fonctionnellement identiques (copies délibérées, cohérentes) — il n'a été découvert qu'au moment où `auth.py` de `tenders_api` a gagné des fonctions (`associate_mfa`/`verify_mfa`) absentes des copies des autres Lambdas, et que les tests correspondants ont échoué avec `AttributeError` uniquement en run combiné (`pytest backend/`), jamais en run isolé (`pytest backend/tenders_api/`).

**Correctif définitif** : ne **jamais** lancer `pytest` sur plusieurs répertoires de Lambdas en un seul process (`pytest backend/`) dès qu'elles partagent des noms de fichiers — lancer un process pytest **séparé par Lambda** (`backend/Makefile` → `make test`, qui boucle et lance `pytest` indépendamment dans chaque sous-dossier). Un process séparé garantit un `sys.modules` propre, donc aucune ambiguïté possible, sans toucher au code de production (pas de passage en vrai package Python, qui casserait le format flat-file attendu par le zip Lambda).

**Règle générale (mise à jour)** : `--import-mode=importlib` seul ne suffit **jamais** à isoler des Lambdas qui partagent des noms de modules — c'est un correctif pour la collecte pytest, pas pour l'isolation d'import. Dans un mono-repo multi-Lambda avec imports plats, le seul run réellement fiable est un process pytest par service. Un run combiné qui "passe" ne prouve rien : il peut tester silencieusement le mauvais module tant que les fichiers concernés n'ont pas encore divergé — la seule façon de le détecter est de faire diverger volontairement un fichier de test entre deux runs (isolé vs combiné) et comparer, ou plus simplement, ne jamais faire de run combiné du tout.
